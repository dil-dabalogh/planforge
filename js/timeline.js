window.PlanForgeTimeline = (function() {
  function create(state, canvas) {
    ...
    let timelineConfig = {
      start: window.PlanForgeModel.today(),
      end: window.PlanForgeModel.addDays(window.PlanForgeModel.today(), 90),
      zoomLevel: 2 // 0=Year, 1=Quarter, 2=Month, 3=Week, 4=Day
    };

    // Zoom level mapping
    const zoomLevels = [
      { name: 'Year', granularity: 'year', pixelsPerUnit: 100, daysPerUnit: 365 },
      { name: 'Quarter', granularity: 'quarter', pixelsPerUnit: 80, daysPerUnit: 90 },
      { name: 'Month', granularity: 'month', pixelsPerUnit: 60, daysPerUnit: 30 },
      { name: 'Week', granularity: 'week', pixelsPerUnit: 40, daysPerUnit: 7 },
      { name: 'Day', granularity: 'day', pixelsPerUnit: 20, daysPerUnit: 1 }
    ];
    ...
    function resizeCanvas() {
      ...
      const daysSpan = Math.max(30, Math.round((new Date(timelineConfig.end) - new Date(timelineConfig.start)) / 86400000));
      ...
      canvas.width = contentWidthCss * dpr;
    ...
    function onPointerMove(ev) {
      ...
      autoScrollTimeline(px, rect.width);
      ...
      if (dragging.isScenario) {
        ...
        const newStart = xToDate(px - dragging.offsetX);
